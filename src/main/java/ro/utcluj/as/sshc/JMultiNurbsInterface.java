...
public class JMultiNurbsInterface {
	...
	public JMultiNurbsInterface() {
		...
		ShapeControl.add(modifpoints);ShapeControl.add(modifknots);
		ShapeControl.add(closecurve); 
		ShapeControl.add(rend); ShapeControl.add(render);
		ShapeControl.add(ord); ShapeControl.add(order);
		PointControl.add(showpoint); PointControl.add(shownumber); 
		PointControl.add(controlPointLabel);
		PointControl.add(controlPoint);	PointControl.add(weightLabel); 
		PointControl.add(scrollWeight); PointControl.add(weight); 
		PointControl.add(back1);
		KnotControl.add(knotLabel); KnotControl.add(knotId); 
		KnotControl.add(scrollKnot); KnotControl.add(prevKnot); 
		KnotControl.add(knotValue); KnotControl.add(nextKnot);
	...
	public void addListeners() {
		...
		addNurbs.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
		...
		idNurbs.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			MultiNurb.switchTo(idNurbs.getSelectedIndex()); 
			initComponents();
		} });
		
		removeNurbs.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			int aux = MultiNurb.removeCurrentNurbs(); 
			idNurbs.removeAllItems(); 
			for(int i=1; i<=aux;i++) idNurbs.addItem(String.valueOf(i)); 
			if (aux>0) {
				MultiNurb.switchTo(0);
				MultiNurb.setDrawing(true);
			}
			else MultiNurb.setDrawing(false);
			MultiNurb.setEditing(false);
			initComponents();
		} });
		...
		back1.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			MultiNurb.setEditing(false); MultiNurb.setDrawing(true);
			ControlToolBar.setVisible(false); ControlToolBar.removeAll(); 
			ControlToolBar.add(ShapeControl); ControlToolBar.setVisible(true); 
		} });

		back2.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			MultiNurb.setEditing(false); MultiNurb.setDrawing(true);
			ControlToolBar.setVisible(false); ControlToolBar.removeAll(); 
			ControlToolBar.add(ShapeControl); ControlToolBar.setVisible(true); 
		} });
		
		closecurve.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) {  closeCurve();  } });
	
		showpoint.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			if (MultiNurb.thereIsActive()) { 
				MultiNurb.getCurrentNurbs().showPoints = !MultiNurb.getCurrentNurbs().showPoints; 
				MultiNurb.repaint(); 
			} 
		} });

		shownumber.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { 
			if (MultiNurb.thereIsActive()) { 
				MultiNurb.getCurrentNurbs().showNumbers = !MultiNurb.getCurrentNurbs().showNumbers;
		 		MultiNurb.repaint(); 
		 	} 
		 } });

		scrollWeight.addAdjustmentListener(new AdjustmentListener() { public void adjustmentValueChanged(AdjustmentEvent e) { scrollWeightChanged(); } });
		scrollKnot.addAdjustmentListener(new AdjustmentListener() { public void adjustmentValueChanged(AdjustmentEvent e) { scrollKnotChanged(); } });
	
		render.addKeyListener (new java.awt.event.KeyAdapter () { public void keyPressed (java.awt.event.KeyEvent evt) { enterHitInRender(evt); }	});
		order.addKeyListener (new java.awt.event.KeyAdapter () { public void keyPressed (java.awt.event.KeyEvent evt) { enterHitInOrder(evt); }	});
		controlPoint.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { controlPointComboChanged(); } });
		knotId.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { knotComboChanged(); } });
		weight.addKeyListener (new java.awt.event.KeyAdapter () { public void keyPressed (java.awt.event.KeyEvent evt) { enterHitInWeight(evt); } });
		knotValue.addKeyListener (new java.awt.event.KeyAdapter () { public void keyPressed (java.awt.event.KeyEvent evt) { enterHitInKnotValue(evt); }	});
		
//		loadNurbs.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { loadNurbs(); } });
//		saveNurbs.addActionListener(new ActionListener() { public void actionPerformed(ActionEvent e) { saveNurbs(); } });
	}
	...
	private void initComponents() {
		...
		d.width = 100; scrollWeight.setMaximum(160); scrollWeight.setMinimum(-150);
		scrollWeight.setValue(0);	scrollWeight.setPreferredSize(d);		
		scrollKnot.setMaximum(137); scrollKnot.setMinimum(1); scrollKnot.setPreferredSize(d);
		render.setText(""); order.setText("");
	...
	private void knotComboChanged() {
		try {
			...
			double min = MultiNurb.getCurrentNurbs().getKnot(i-1), max = MultiNurb.getCurrentNurbs().getKnot(i+1), x = MultiNurb.getCurrentNurbs().getKnot(i);
	...
	private void enterHitInWeight(java.awt.event.KeyEvent evt) {
		if(evt.getKeyCode() == evt.VK_ENTER) {
			try {
				...
				if (currentNurbs.modifyPoint(i,currentNurbs.getControlPoint(i),w)) currentNurbs.repaint();  
				else { weight.setText(p.getw().toString()); }			 		
			}
			catch (Exception e) { }
		}
	}
	...
	private void enterHitInKnotValue(java.awt.event.KeyEvent evt) {
		if(evt.getKeyCode() == evt.VK_ENTER) {
			try {
				...
				if (currentNurbs.setKnot(i,w)) currentNurbs.repaint();  
				else knotValue.setText(String.valueOf(p)); 			 		
			}
			catch (Exception e) { }
		}
	}
	...
	private void scrollKnotChanged() {
		try {
			...
			double min = MultiNurb.getCurrentNurbs().getKnot(i-1), max = MultiNurb.getCurrentNurbs().getKnot(i+1);
	...
	private void loadNurbs() {
		try {
			java.io.ObjectInputStream objInStream = new java.io.ObjectInputStream(new java.io.FileInputStream("nurbs.tmp"));
			JMultiNurbs temp = (JMultiNurbs)objInStream.readObject();
			objInStream.close();		
				MultiNurb = temp;
				idNurbs.removeAllItems(); 
				for(int i=1; i<=MultiNurb.nurbsCount();i++) idNurbs.addItem(String.valueOf(i));				
				MultiNurb.addMouseClickListener();
				initComponents();
		}
		catch (Exception e) {
	...
	public void saveNurbs(java.io.File file) {
		try {
			java.io.ObjectOutputStream objOutStream = new java.io.ObjectOutputStream(new java.io.FileOutputStream(file));
...
class JMultiNurbs extends javax.swing.JPanel implements java.io.Serializable  {
	...
	public void redrawCurrentNurbsWith(String what, int index, double newvalue) {
		if (currentNurbs!=null) {
			if (what=="ControlPoint") currentNurbs.modifyPoint(index, newvalue);
			if (what=="Knot") currentNurbs.setKnot(index, newvalue);
			currentNurbs.repaint();
		}		
	}
	...
	public void setCloseCurrentCurve(boolean what) {
		if (currentNurbs!=null) currentNurbs.setClosedCurve(what);
	}
	
	public boolean getCloseCurrentCurve() {
		if (currentNurbs!=null) return currentNurbs.isClosed();
		else return false;
	}
	...
	public java.util.Vector toPolyline() {
		...
		for(int i=0;i<Nurbs.size();i++) {
			...
			if (o!=null) polyline.add(o);
		}
		return polyline;	
	}
	
	public boolean thereIsActive() {
		if (currentNurbs!=null) return true;
		else return false;
	}
