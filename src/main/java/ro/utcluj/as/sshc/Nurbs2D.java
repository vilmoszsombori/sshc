...
public class Nurbs2D extends javax.swing.JComponent implements java.io.Serializable {
    ...
    private static final long serialVersionUID = -1998128398114614210L;
    ...
    private int render;
    private int order;
    private Vector<Point2D> controlPoint;
    private Vector<Point> poligon;
    private double[] knot;
    ...
    public Point2D getControlPoint(int i) {
        if ((i >= 0) && (i < controlPoint.size()))
            return controlPoint.elementAt(i);
        else
            return null;
    }
    ...
    public Vector<Point> toPolyline() {
        if (controlPoint.size() > 0) {
            if (!curveClosed) {
                curveClosed = true;
                updatePolygon();
                for (int i = 0; i < render; i++)
                    poligon.remove(poligon.size() - 1);
            }
            return poligon;
        } else
            return null;
    ...
    public void paint(java.awt.Graphics g) {
        ...
        if (showPoints) {
            ...
            for (int i = 0; i < controlPoint.size(); i++) {
                ...
                if (showNumbers)
                    g.drawString(String.valueOf(i), x, y - 3);
            }
        }
        if (controlPoint.size() > order) {
            ...
            g.drawString("Order must be lower than the number of control points!", 10, 10);
    }
    ...
    private double N(int i, int k, double t) {
        if (k == 1) {
            ...
            return ((t - knot[i]) * N(i, k - 1, t) / (knot[i + k - 1] - knot[i]))
                    + ((knot[i + k] - t) * N(i + 1, k - 1, t) / (knot[i + k] - knot[i + 1]));
        }
    }
    ...
    private void init() {
        ...
        showNumbers = false;
        curveClosed = false;
        controlPoint = new Vector<>();
        poligon = new Vector<>();
        knot = new double[MAXNRPOINT + MAXORDER];
        for (int i = 0; i < MAXNRPOINT + MAXORDER; i++)
            knot[i] = i;
    }
