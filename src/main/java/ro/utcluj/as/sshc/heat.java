...
class BoxMotionAdapter extends MouseMotionAdapter {
	public void mouseMoved(MouseEvent m) {
		...
		if ((i!=-1)&&(j!=-1)) {
			...
			try  { center.remove(box); center.repaint(); 
			}catch(Exception e) {}
		}
	}
}
...
class InsertListener extends MouseAdapter {
		public void mousePressed(MouseEvent e) {
			...
			if (statistics.isSelected()) {
				...
				domain = mesher.getCompDomainGrid(xm, ym, dom, grid, num,
...
//				t.setText(((JMenuItem)e.getSource()).getText());
	...
	private void addCheckListeners() {
		...
		{ public void keyPressed (java.awt.event.KeyEvent evt) 
			{ if(evt.getKeyCode() == evt.VK_ENTER) refineMesh(); }
		});

		vertexVisibility.addKeyListener (new java.awt.event.KeyAdapter () 
		{ public void keyPressed (java.awt.event.KeyEvent evt) 
			{ if(evt.getKeyCode() == evt.VK_ENTER) chVisibleVertex(); }
		});

		edgeVisibility.addKeyListener (new java.awt.event.KeyAdapter () 
		{ public void keyPressed (java.awt.event.KeyEvent evt) 
			{ if(evt.getKeyCode() == evt.VK_ENTER) chVisibleEdge(); }
		});
	}
	...
	private void openDialog() {
		...
		if (dataFile!=null) fc.setCurrentDirectory(dataFile);
		int returnVal = fc.showOpenDialog(this);
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			dataFile = fc.getSelectedFile();
			JMultiNurbs temp = null;
			try {
				java.io.ObjectInputStream objInStream = new java.io.ObjectInputStream(new java.io.FileInputStream(dataFile));
...
//				JComponent a = mesher.getCompDomain();
	...
	private void createOrdinaryMesh(double a) {
		...
		}catch (Exception e)  { System.out.println("exp in ordinary mesh");}
	}
...
//				JComponent a = mesher.getCompDomain();
		...
		}catch (Exception e)  { System.out.println("exp in custom mesh");}
	}
	...
	private void centerSetCmesh() {
				if (statistics.isSelected()) {
					...
					if (!viewMesh) stat.addMouseListener(insertListener);
					center.add(stat);
					center.repaint();
					center.setVisible(true);	
				}
					...
					domain = mesher.getCompDomainGrid(xm, ym, dom, grid, num,
					  ...
					  showGridNumbers.isSelected(), showGrid.isSelected(), ycoord);	
					domain.addMouseMotionListener(triangleMotion);
					if (!viewMesh) domain.addMouseListener(insertListener);
					center.add(domain);
					center.repaint();
					center.setVisible(true);	
				}
	...
	private void centerSetGeom() {
		center.setVisible(false);
		center.removeAll();
		domain = geomer.getCompDomainGrid(xm, ym, dom, grid, num,
		  showEdge.isSelected(), showVertex.isSelected(),
		  showGridNumbers.isSelected(), showGrid.isSelected(), ycoord);	
	...
	private void updateTriangle() {
		...
		try {
			t = query;
			if (includingTriangle>0) query = mesher.getCompTriangle(xm,ym, triangle, circle, ycoord);
			else
			if (includingTriangle<0) query = mesher.getCompHole(includingTriangle,xm,ym, triangle, ycoord);
			else query = null;	
		}
		catch (Exception e) { }
		center.setVisible(false);
		if (t!=null) center.remove(t);
		if (query!=null) center.add(query, -1);
		center.repaint();
		center.setVisible(true);	
	}
	...
	private void refineMesh() {
				try  {
					...
					if ((i<35.0)&&(i>0)) {
						try { south.remove(removeVertex); }catch(Exception exp)  {}
						mesher.refine(i);
						center.setVisible(false);
						center.removeAll();
						if (statistics.isSelected()) {
        ...
        JOptionPane pane = new JOptionPane(array, 
		 ...
		 if (selectedValue==btnString1) {
			...
			try {
				ecuation.kx = (new Double(kx.getText())).doubleValue();
				ecuation.ky = (new Double(ky.getText())).doubleValue();
				TwoVariableFunction t = new TwoVariableFunction(boundaryFunction.getText(), "x", "y");
			 ...
			 catch(Exception exp)  { ggboundary(); }
		 }
	}
...
class SourceListener extends MouseAdapter {
	...
	private void gsource() {
		...
		try {
			...
			if (v.size()>0) {
					...
					if (sourceGraphic!=null)  {
						if (ecuation.source.restCloseToMin()) tip = Color.white;
		...
		else space.setBackground(Color.white);
